...
module.exports = function historyPlugin(schema, options) {
  ...
  schema.pre('findOneAndUpdate', async function (next) {
    ...
    var original = await this.model.findOne(this.getQuery());

    if(diffOnly) {
      historyDoc = createHistoryDoc(this._update.$set, 'u');
    } else {      
      var d = original.toObject();
      var keys = Object.keys(this._update.$set);
      for(var i = 0; i < keys.length; i++) {
        d[ keys[i] ] = this._update.$set[ keys[i] ];
      }
  ...
  schema.pre('remove', async function(next) {
    var original = await this.model.findOne(this.getQuery());
    let d = original.toObject();
    let historyDoc = createHistoryDoc(d, 'r');

    saveHistoryModel(original.toObject(), d, historyDoc, this.collection.name, next);
  });
  ...
  async function processUpdate(next) {
    ...
    var original = await this.model.findOne(this.getQuery());
    saveHistoryModel(original.toObject, d, historyDoc, this.mongooseCollection.collectionName, next);
  }

  async function processRemove(doc, next) {
